...
Terminal-Bench 2.0 的一个更轻量、更快速的替代方案，用于迭代终端智能体。
使用与 TerminalBench2EvalEnv 相同的评估逻辑，但默认使用
NousResearch/openthoughts-tblite 数据集（100 个难度校准任务，相比 TB2 的 89 个更难的任务）。

TBLite 任务是 TB2 的精选子集，其难度分布旨在即使对较小的模型也能提供有意义的信号：
...
class TBLiteEvalConfig(TerminalBench2EvalConfig):
    """OpenThoughts-TBLite 评估环境的配置类。

    继承所有 TB2 配置字段。仅数据集默认值和任务超时时间不同——
    TBLite 任务被校准为更快完成。
    """
...
class TBLiteEvalEnv(TerminalBench2EvalEnv):
    ...
    继承 TerminalBench2EvalEnv 的所有评估逻辑（智能体循环、
    测试验证、Docker 镜像解析、指标、wandb 日志记录）。
    仅默认配置不同。
    """
