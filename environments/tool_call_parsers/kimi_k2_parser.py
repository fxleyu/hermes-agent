...
function_id 的格式通常为 "functions.func_name:index" 或 "func_name:index"。
...
class KimiK2ToolCallParser(ToolCallParser):
    ...
    使用 section begin/end 标记包装各个 tool call begin/end 标记。
    tool_call_id 中包含函数名（最后一个点之后、冒号之前的部分）。
    """
    ...
    # 正则表达式捕获：tool_call_id（例如 "functions.get_weather:0"）和 function_arguments
    PATTERN = re.compile(
