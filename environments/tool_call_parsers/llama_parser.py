...
可能以 <|python_tag|> 标记开头。支持多个以内容或分号分隔的 JSON 对象。
...
class LlamaToolCallParser(ToolCallParser):
    """
    Llama 3.x 和 4 JSON 格式工具调用的解析器。

    查找包含 "name" + ("arguments" 或 "parameters") 键的 JSON 对象。
    使用 Python 的 json.JSONDecoder.raw_decode 从混合文本中
    鲁棒地提取 JSON 对象。
    """

    BOT_TOKEN = "<|python_tag|>"

    # 用于查找潜在 JSON 对象起始位置的正则表达式
    JSON_START = re.compile(r"\{")

    def parse(self, text: str) -> ParseResult:
        # 快速检查：需要 bot token 或 JSON 大括号
        if self.BOT_TOKEN not in text and "{" not in text:
            return text, None

        try:
            # 使用 raw_decode 可以从任意位置开始解析 JSON，不要求整个字符串都是 JSON
            decoder = json.JSONDecoder()
            tool_calls: List[ChatCompletionMessageToolCall] = []
            end_index = -1  # 追踪上一个已解析 JSON 的结束位置

            for match in self.JSON_START.finditer(text):
                start = match.start()
                # 跳过位于已解析 JSON 对象内部的大括号
                if start <= end_index:
                    continue
                ...
                try:
                    ...
                    # 必须包含 "name" 以及 "arguments" 或 "parameters" 之一
                    name = obj.get("name")
