...
sidebar_position: 15
title: "自动化模板"
description: "开箱即用的自动化方案 — 定时任务、GitHub 事件触发器、API Webhook 和多技能工作流"
---

# 自动化模板

常见自动化模式的可复制粘贴方案。每个模板使用 Hermes 的内置 [cron 调度器](/docs/user-guide/features/cron)进行基于时间的触发，使用 [webhook 平台](/docs/user-guide/messaging/webhooks)进行事件驱动触发。

每个模板适用于**任何模型** — 不锁定于单一提供商。
...
所有三种都支持投递到 Telegram、Discord、Slack、SMS、邮件、GitHub 评论或本地文件。
:::
...
每晚标记、排序和总结新的 issue。向你的团队频道投递摘要。
...
在 PR 打开时自动审查每个拉取请求。直接在 PR 上发布审查评论。
...
每次部署后触发冒烟测试。你的 CI/CD 管道在部署完成时向 webhook POST。
...
将监控告警与最近的变更关联起来，草拟响应方案。适用于 Datadog、PagerDuty、Grafana 或任何能 POST JSON 的告警系统。
...
| SMS | `--deliver sms:+15551234567` | 直接发到手机号 |
...
| `{__raw__}` | 完整 JSON 负载（截断为 4000 字符） |
| `{sender.login}` | 触发事件的 GitHub 用户 |

### [SILENT] 模式

当 cron 任务的回复包含 `[SILENT]` 时，投递被抑制。使用此方式避免安静运行时的通知垃圾：

```
If nothing noteworthy happened, respond with [SILENT].
```

这意味着你只在代理有东西要报告时才收到通知。
