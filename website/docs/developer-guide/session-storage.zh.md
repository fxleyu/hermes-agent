...
Hermes Agent 使用 SQLite 数据库（`~/.hermes/state.db`）来持久化会话元数据、完整消息历史和模型配置，跨越 CLI 和网关会话。这取代了早期的逐会话 JSONL 文件方式。
...
关键设计决策：
- **WAL 模式**用于并发读取 + 单个写入者（网关多平台）
- **FTS5 虚拟表**用于跨所有会话消息的快速文本搜索
- **会话血统**通过 `parent_session_id` 链（压缩触发的分裂）
- **来源标记**（`cli`、`telegram`、`discord` 等）用于平台过滤
- 批处理运行器和 RL 轨迹不存储在此（独立系统）
...
FTS5 表通过三个触发器保持同步，在 `messages` 表的 INSERT、UPDATE 和 DELETE 时触发：
...
每个迁移使用 `ALTER TABLE ADD COLUMN` 包装在 try/except 中以处理列已存在的情况（幂等）。每个成功迁移块后版本号递增。


## 写入竞争处理

多个 hermes 进程（网关 + CLI 会话 + worktree 代理）共享一个 `state.db`。`SessionDB` 类通过以下方式处理写入竞争：

- **短 SQLite 超时**（1 秒）而非默认的 30 秒
- **应用级重试**，带随机抖动（20-150ms，最多 15 次重试）
- **BEGIN IMMEDIATE** 事务，在事务开始时暴露锁竞争
- 每 50 次成功写入的**周期性 WAL 检查点**（PASSIVE 模式）

这避免了"护送效应"，即 SQLite 的确定性内部退避导致所有竞争写入者在相同间隔重试。
...
db.create_session(
    ...
    user_id="user_1",
    parent_session_id=None,  # 或前一个会话 ID 用于血统
)
...
这由 `hermes_constants.get_hermes_home()` 派生，默认解析为 `~/.hermes/`，或 `HERMES_HOME` 环境变量的值。
