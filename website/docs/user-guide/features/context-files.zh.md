...
Hermes Agent 自动发现和加载影响其行为的上下文文件。有些是项目本地的，从你的工作目录中发现。`SOUL.md` 现在是 Hermes 实例的全局文件，仅从 `HERMES_HOME` 加载。
...
| **CLAUDE.md** | Claude Code 上下文文件（同样被检测） | 启动时的 CWD + 逐步发现子目录 |
...
:::info 优先级系统
每个会话只加载**一种**项目上下文类型（首次匹配优先）：`.hermes.md` -> `AGENTS.md` -> `CLAUDE.md` -> `.cursorrules`。**SOUL.md** 始终作为代理身份（插槽 #1）独立加载。
:::

## AGENTS.md

`AGENTS.md` 是主要的项目上下文文件。它告诉代理你的项目如何组织、应遵循哪些约定以及任何特殊指令。

### 渐进式子目录发现

会话开始时，Hermes 将工作目录中的 `AGENTS.md` 加载到系统提示中。当代理在会话期间（通过 `read_file`、`terminal`、`search_files` 等）导航到子目录时，它会**渐进式发现**这些目录中的上下文文件，并在它们变得相关时注入对话。
...
- **不会导致系统提示膨胀** ——子目录提示仅在需要时出现
- **保留提示缓存** ——系统提示在各轮对话中保持稳定

每个子目录在每个会话中最多检查一次。发现机制也会向上遍历父目录，因此读取 `backend/src/main.py` 即使 `backend/src/` 没有自己的上下文文件也会发现 `backend/AGENTS.md`。

:::info
子目录上下文文件与启动上下文文件经过相同的[安全扫描](#安全提示注入防护)。恶意文件会被阻止。
:::
...
## 架构
- 前端：Next.js 14 App Router 在 `/frontend`
...
`SOUL.md` 控制代理的个性、语调和沟通风格。详见[个性](/docs/user-guide/features/personality)页面。
...
- 如果尚不存在，Hermes 会自动创建默认的 `SOUL.md`
- Hermes 仅从 `HERMES_HOME` 加载 `SOUL.md`
- Hermes 不会在当前工作目录中查找 `SOUL.md`
- 如果文件为空，`SOUL.md` 不会向提示中添加任何内容
- 如果文件有内容，内容在扫描和截断后逐字注入

## .cursorrules

Hermes 兼容 Cursor IDE 的 `.cursorrules` 文件和 `.cursor/rules/*.mdc` 规则模块。如果这些文件存在于你的项目根目录中且没有找到更高优先级的上下文文件（`.hermes.md`、`AGENTS.md` 或 `CLAUDE.md`），它们将作为项目上下文加载。
...
3. **安全扫描** ——检查内容中的提示注入模式
4. **截断** ——超过 20,000 个字符的文件进行头尾截断（70% 头部、20% 尾部，中间有标记）
...
所有上下文文件在包含之前都会进行潜在提示注入扫描。扫描器检查：
...
:::warning
此扫描器可以防范常见的注入模式，但它不能替代在共享仓库中审查上下文文件。始终验证非你编写的项目中的 AGENTS.md 内容。
:::
...
:::tip AGENTS.md 最佳实践
1. **保持简洁** ——保持在 20K 字符以下；代理每轮都会读取它
2. **使用标题结构化** ——使用 `##` 部分划分架构、约定、重要说明
3. **包含具体示例** ——展示首选的代码模式、API 结构、命名约定
4. **提及不该做的事** ——"永远不要直接修改迁移文件"
5. **列出关键路径和端口** ——代理将其用于终端命令
6. **随项目发展更新** ——过时的上下文比没有上下文更糟糕
:::

### 子目录级上下文

对于 monorepo，在嵌套的 AGENTS.md 文件中放置子目录特定的指令：
