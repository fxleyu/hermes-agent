...
Hermes Agent 包含一个基于 **Tinker-Atropos** 构建的集成 RL（强化学习）训练流水线。这使得可以使用 GRPO（Group Relative Policy Optimization，组相对策略优化）配合 LoRA 适配器在环境特定任务上训练语言模型，整个过程通过智能体的工具接口编排。
...
智能体可以发现环境、配置训练参数、启动训练运行并监控指标 —— 全部通过一组 `rl_*` 工具完成。
...
- `tinker-atropos` 子模块（位于 Hermes 根目录的 `tinker-atropos/` 相对路径下）
...
智能体调用 `rl_list_environments()`，该方法使用 AST 解析扫描 `tinker-atropos/tinker_atropos/environments/`，查找继承自 `BaseEnv` 的 Python 类。每个环境定义了：
...
智能体调用 `rl_start_training()`，它会：

1. 生成一个 YAML 配置文件，合并锁定设置和可配置的覆盖值
2. 创建一个唯一的运行 ID
3. 生成三个进程：
   - **Atropos API 服务器**（`run-api`）—— 轨迹协调
   - **Tinker 训练器**（`launch_training.py`）—— LoRA 训练 + 端口 8001 上的 FastAPI 推理服务器
   - **环境**（`environment.py serve`）—— 连接到 Atropos 的所选环境
...
- 运行时间
- WandB 指标（步数、平均奖励、正确率、评估准确率）
- 用于调试的日志文件位置

:::note 速率限制
状态检查对每个运行 ID 限制为每 **30 分钟**一次。这防止了在需要数小时的长时间训练任务中过度轮询。
:::
...
在进行完整训练运行之前，你可以使用 `rl_test_inference` 测试环境是否正常工作。这使用 OpenRouter 运行几个步骤的推理和评分 —— 不需要 Tinker API，只需要一个 `OPENROUTER_API_KEY`。
...
默认配置：
- **3 步 x 16 个补全 = 每个模型 48 次 rollout**
- 测试 3 个不同规模的模型以确保鲁棒性：
  - `qwen/qwen3-8b`（小型）
...
- **训练客户端** —— 处理具有重要性采样损失的前向-反向传播、优化器步骤（Adam）和权重检查点
- **采样客户端** —— 使用最新训练权重提供推理

训练循环：
1. 从 Atropos 获取一批 rollout（提示词 + 补全 + 分数）
2. 转换为带有填充 logprobs 和优势值的 Tinker Datum 对象
3. 运行具有重要性采样损失的前向-反向传播
...
3. 实现所需的方法：
   - `load_dataset()` —— 加载训练数据
   - `get_next_item()` —— 为模型提供下一个条目
   - `score_answer()` —— 评分模型输出并分配奖励
   - `collect_trajectories()` —— 收集并返回轨迹
4. 可选择定义一个继承自 `BaseEnvConfig` 的自定义配置类

参考现有的 `gsm8k_tinker.py` 作为模板。智能体可以帮助你创建新环境 —— 它可以读取现有环境文件、检查 HuggingFace 数据集并编写新的环境代码。
...
每次训练运行会在 `~/.hermes/logs/rl_training/` 中生成日志文件：
