...
description: "发送邮件、管理日历事件、搜索 Drive、读写 Sheets 和访问 Docs——全部通过 OAuth2 认证的 Google API"
---

# Google Workspace 技能

为 Hermes 集成 Gmail、日历、Drive、通讯录、Sheets 和 Docs。使用 OAuth2 并自动刷新令牌。优先使用 [Google Workspace CLI (`gws`)](https://github.com/nicholasgasior/gws)（可用时）以获得更广泛的覆盖，否则回退到 Google 的 Python 客户端库。

**技能路径：** `skills/productivity/google-workspace/`

## 设置

设置完全由智能体驱动——让 Hermes 设置 Google Workspace，它会引导你完成每个步骤。流程：

1. **创建 Google Cloud 项目**并启用所需 API（Gmail、日历、Drive、Sheets、Docs、People）
2. **创建 OAuth 2.0 凭据**（桌面应用类型）并下载客户端密钥 JSON
3. **授权** —— Hermes 生成授权 URL，你在浏览器中批准，粘贴回重定向 URL
4. **完成** —— 之后令牌自动刷新

:::tip 仅邮件用户
如果你只需要邮件（不需要日历/Drive/Sheets），使用 **himalaya** 技能——它使用 Gmail 应用密码，只需 2 分钟。不需要 Google Cloud 项目。
:::
...
返回 JSON，每条消息包含 `id`、`from`、`subject`、`date`、`snippet` 和 `labels`。
...
`--from` 标志让你自定义发送邮件的发件人显示名称。当多个智能体共享同一 Gmail 账号但你想让收件人看到不同名称时很有用：
...
**工作原理：** `--from` 值作为 RFC 5322 `From` 头设置在 MIME 消息上。Gmail 允许在你自己的认证邮箱地址上自定义显示名称，无需任何额外配置。收件人看到自定义显示名称（例如"Research Agent"），而邮箱地址保持不变。

**重要：** 如果你在 `--from` 中使用*不同的邮箱地址*（不是认证账号），Gmail 要求该地址在 Gmail 设置 → 账号 → 发送邮件身份中配置为[发送身份别名](https://support.google.com/mail/answer/22370)。
...
自动将回复加入线程（设置 `In-Reply-To` 和 `References` 头）并使用原始消息的线程 ID。
...
:::warning
日历时间**必须**包含时区偏移（例如 `-07:00`）或使用 UTC（`Z`）。裸时间如 `2026-03-01T10:00:00` 是模糊的，将被视为 UTC。
:::
