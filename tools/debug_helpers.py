"""Hermes 工具的共享调试会话基础设施。

替代了之前在 web_tools、vision_tools、mixture_of_agents_tool 和
image_generation_tool 中重复的相同 DEBUG_MODE / _log_debug_call /
_save_debug_log / get_debug_session_info 样板代码。
...
class DebugSession:
    """逐工具的调试会话，将工具调用记录到 JSON 日志文件中。

    通过工具特定的环境变量激活（例如 WEB_TOOLS_DEBUG=true）。
    禁用时，所有方法都是低开销的空操作。
    """
    ...
    def get_session_info(self) -> Dict[str, Any]:
        """返回适合由 get_debug_session_info() 返回的摘要字典。"""
        if not self.enabled:
            return {
                "enabled": False,
                "session_id": None,
