...
本模块提供通过 Tinker-Atropos 运行强化学习训练的工具。
...
# tinker-atropos 子模块的路径（相对于 hermes-agent 根目录）
    ...
    "openai": [
        {
            ...
            "timeout": 3600,
            "server_type": "sglang",  # Tinker 使用 sglang 进行实际训练
        }
    ],
    "tinker": {
        "lora_rank": 32,
...
class RunState:
    ...
    environment: str
    config: Dict[str, Any]
    status: str = "pending"  # pending（待处理）、starting（启动中）、running（运行中）、stopping（停止中）、stopped（已停止）、completed（已完成）、failed（失败）
    error_message: str = ""
    wandb_project: str = ""
    wandb_run_name: str = ""
...
async def _spawn_training_run(run_state: RunState, config_path: Path):
    ...
    try:
        # 第一步：启动 Atropos API 服务器 (run-api)
        logger.info("[%s] Starting Atropos API server (run-api)...", run_id)
        
        # 文件必须在子进程运行期间保持打开；我们将句柄存储在
        # run_state 上，以便 _stop_training_run() 完成时可以关闭它。
        api_log_file = open(api_log, "w")  # 由 _stop_training_run 关闭
        ...
        trainer_log_file = open(trainer_log, "w")  # 由 _stop_training_run 关闭
        ...
        # 等待训练器初始化（它在端口 8001 上启动 FastAPI 推理服务器）
        ...
        env_log_file = open(env_log, "w")  # 由 _stop_training_run 关闭
...
async def rl_list_environments() -> str:
    ...
    扫描 tinker-atropos/tinker_atropos/environments/ 目录中
    包含继承自 BaseEnv 的类的 Python 文件。

    返回每个环境的信息，包括：
    - name: 环境标识符
    - class_name: Python 类名
    - file_path: 环境文件的路径
    - description: 简要描述（如果可用）

    提示：要创建或修改 RL 环境：
    1. 使用终端/文件工具检查现有环境
    2. 研究它们如何加载数据集、定义验证器和构建奖励结构
...
async def rl_select_environment(name: str) -> str:
    ...
    返回：
        包含选择结果、文件路径和可配置字段数量的 JSON 字符串

    提示：阅读返回的 file_path 以了解环境的工作原理。
    """
    ...
    # 自动设置 wandb_name 为 "{env_name}-日期时间" 以避免重叠
...
async def rl_get_current_config() -> str:
    ...
    字段分为：
    - configurable_fields: 可通过 rl_edit_config() 更改
    - locked_fields: 不可更改的基础设施设置
...
async def rl_edit_config(field: str, value: Any) -> str:
    ...
    先使用 rl_get_current_config() 查看所选环境的可用字段。
    每个环境有不同的选项。
...
async def rl_start_training() -> str:
    ...
    需要先使用 rl_select_environment() 选择环境。
    启动前使用 rl_edit_config() 调整配置。
...
async def rl_check_status(run_id: str) -> str:
    ...
    速率限制：对于长时间运行的训练，此函数对同一 run_id
    强制执行最少 30 分钟的检查间隔。

    参数：
        run_id: rl_start_training() 返回的运行 ID
...
# ============================================================================
# 推理测试（通过 Atropos `process` 模式使用 OpenRouter）
# ============================================================================

# 不同规模的测试模型，用于健壮性测试
# 这些是 OpenRouter 上便宜、能力不错的模型，用于测试解析/评分
TEST_MODELS = [
...
# 默认测试参数 - 快速但有代表性
DEFAULT_NUM_STEPS = 3       # 测试的步数（项目数）
DEFAULT_GROUP_SIZE = 16     # 每个项目的补全数（与训练类似）
...
async def rl_test_inference(
    ...
    """
    任何环境的快速推理测试，使用 Atropos 的 `process` 模式。
    ...
    默认：3 步 x 16 补全 = 每个模型 48 次 rollout。
    测试 3 个模型 = 共 144 次 rollout。快速健全性检查。

    测试模型（不同智能水平以测试健壮性）：
    - qwen/qwen3-8b（小型）
    - zhipu-ai/glm-4-flash（中型）
    - minimax/minimax-m1（大型）

    参数：
        num_steps: 运行步数（默认：3，测试建议最大值）
        group_size: 每步补全数（默认：16，与训练相同）
        models: 可选的模型 ID 列表。如果为 None，使用全部 3 个测试模型。

    返回：
        包含每个模型结果的 JSON：测试步数、准确率、分数
    """
    if not _current_env:
        return json.dumps({
            "error": "No environment selected. Use rl_select_environment(name) first.",
    ...
    for model_info in test_models:
        ...
        # 使用 Atropos 内置 CLI 构建 process 命令
        # 这使用 OpenRouter 作为推理后端运行环境的实际代码
        # 我们通过 CLI 参数传递锁定设置 + 测试特定的覆盖
        cmd = [
            ...
            # 用于推理测试的 OpenRouter 配置
            # 重要：OpenRouter 使用 server_type=openai（不是 sglang）
            # sglang 仅用于 Tinker 推理服务器的实际训练
            "--openai.base_url", "https://openrouter.ai/api/v1",
            "--openai.api_key", api_key,
            "--openai.model_name", model_id,
            "--openai.server_type", "openai",  # OpenRouter 兼容 OpenAI 协议
            "--openai.health_check", "false",  # OpenRouter 没有健康检查端点
        ]
...
def check_rl_python_version() -> bool:
    ...
    tinker-atropos 依赖的 'tinker' 包要求 Python >= 3.11。
    """
...
def get_missing_keys() -> List[str]:
    """
    获取 RL 工具缺少的依赖列表（API 密钥和 Python 版本）。
    """
    missing = []
    if not check_rl_python_version():
