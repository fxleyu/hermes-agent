"""Browserbase 云端浏览器提供者（仅支持直接凭证方式）。"""
...
class BrowserbaseProvider(CloudBrowserProvider):
    """Browserbase (https://browserbase.com) 云端浏览器后端。

    此提供者需要直接的 BROWSERBASE_API_KEY 和 BROWSERBASE_PROJECT_ID 凭证。
    托管 Nous 网关支持已移除 - Nous 订阅现在通过 Browser Use 路由。
    """
