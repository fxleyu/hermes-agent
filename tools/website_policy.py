"""具有 URL 功能的工具的网站访问策略辅助模块。

本模块从 ~/.hermes/config.yaml 和可选的共享列表文件加载用户管理的
网站黑名单。它有意设计得轻量级，以便 web/浏览器工具可以执行 URL
策略，而无需引入更重的 CLI 配置栈。

策略以较短的 TTL 缓存在内存中，以便配置更改可以快速生效，
...
# 缓存：已解析的策略 + 时间戳。避免在每次 URL 检查时重新读取
# config.yaml（一次有 50 个页面的 web_crawl 否则意味着 51 次 YAML 解析）。
...
def _iter_blocklist_file_rules(path: Path) -> List[str]:
    ...
    缺失或不可读的文件会记录警告并返回空列表，
    而非抛出异常——一个错误的文件路径不应禁用所有 web 工具。
    """
    try:
...
def load_website_blocklist(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    结果会缓存 ``_CACHE_TTL_SECONDS`` 秒，以避免在每次 URL 检查时
    重新读取 config.yaml。传入显式的 ``config_path`` 可绕过缓存
    （用于测试）。
    """
...
def invalidate_cache() -> None:
    """强制下次 ``check_website_access`` 调用重新读取配置。"""
...
def check_website_access(url: str, config_path: Optional[Path] = None) -> Optional[Dict[str, str]]:
    ...
    如果允许访问返回 ``None``，如果被阻止则返回包含阻止元数据
    （``host``、``rule``、``source``、``message``）的字典。

    策略错误时不会抛出异常——记录警告并返回 ``None``
    （失败开放），以免配置笔误导致所有 web 工具失效。
    传入显式的 ``config_path``（测试）可获得严格的错误传播。
    """
    # 快速路径：如果没有显式 config_path 且缓存的策略已禁用或为空，
    # 跳过所有工作（不读取 YAML，不提取主机名）。
    if config_path is None:
        with _cache_lock:
