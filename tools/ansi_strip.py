...
被 terminal_tool、code_execution_tool 和 process_registry 用于在将命令输出
返回给模型之前清理转义序列。这可以防止 ANSI 码进入模型上下文——模型将转义序列
复制到文件写入中的根本原因就在于此。

覆盖完整的 ECMA-48 规范：CSI（包括私有模式 ``?`` 前缀、冒号分隔参数、中间字节）、
...
def strip_ansi(text: str) -> str:
    ...
    当不存在 ESC 或 C1 字节时直接返回原始输入（快速路径）。
    可安全地对任何字符串调用——干净的文本会以极低的开销直接通过。
    """
    # 如果文本为空或不包含转义字符，直接返回原文（快速路径优化）
